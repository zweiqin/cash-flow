<template>
	<view class="tn-number-box-class tn-number-box">
		...
		<view
			...
			@mousedown.stop.prevent="touchStart('minus')"
			@mouseup.stop.prevent="clearTimer"
		>
		...
		<view
			...
			@mousedown.stop.prevent="touchStart('plus')"
			@mouseup.stop.prevent="clearTimer"
		>
...
export default {
	...
	watch: {
		...
		inputValue(newVal, oldVal) {
			...
			const isNumber = this.$t.test.number(newVal)
	...
	methods: {
		...
		// 处理小数相加减出现溢出问题
		calcPlus(num1, num2) {
			let baseNum = 0; let baseNum1 = 0; let
				baseNum2 = 0
			try {
