<template>
	<view class="components-form">
		...
		<view :style="{paddingTop: vuex_custom_bar_height + 'px'}">
			...
			<dynamic-demo-template ref="demoTemplate" :tips="tips" :section-list="sectionList" :full="true" :full-windows-scroll="true" @click="click">
				<tn-form ref="form" :model="model" :error-type="errorType" :label-position="labelPosition" :label-width="labelWidth" :label-align="labelAlign">
					<tn-form-item label="姓名" prop="name" left-icon="identity" :required="true" :label-position="labelPosition" :label-align="labelAlign">
						<tn-input v-model="model.name" type="text" placeholder="请输入姓名" :border="border"></tn-input>
					</tn-form-item>
					<tn-form-item label="性别" prop="sex" :label-position="labelPosition" :label-align="labelAlign">
						<tn-input v-model="model.sex" type="select" placeholder="请选择性别" :border="border" :select-open="actionSheetShow" @click="actionSheetShow = true"></tn-input>
					</tn-form-item>
					<tn-form-item label="手机号码" prop="phone" right-icon="phone" :label-position="labelPosition" :label-align="labelAlign">
						<tn-input v-model="model.phone" type="number" placeholder="请输入手机号码" :border="border"></tn-input>
					</tn-form-item>
					<tn-form-item label="介绍" prop="desc" :label-position="labelPosition" :label-align="labelAlign">
						<tn-input v-model="model.desc" type="textarea" placeholder="请输入介绍" :border="border" input-align="center"></tn-input>
					</tn-form-item>
					<tn-form-item label="密码" prop="password" :label-position="labelPosition" :label-align="labelAlign">
						<tn-input v-model="model.password" type="password" placeholder="请输入密码" :border="border" :password-icon="true"></tn-input>
					</tn-form-item>
					<tn-form-item label="确认密码" prop="rePassword" :label-position="labelPosition" :label-align="labelAlign">
						<tn-input v-model="model.rePassword" type="password" placeholder="请再次输入密码" :border="border"></tn-input>
					</tn-form-item>
					<tn-form-item label="水果" prop="fruit" :label-position="labelPosition" :label-align="labelAlign">
					...
					<tn-form-item label="支付方式" prop="payType" :label-position="labelPosition" :label-align="labelAlign">
					...
					<tn-form-item label="所在地区" prop="region" :label-position="labelPosition" :label-align="labelAlign">
						<tn-input v-model="model.region" type="select" placeholder="请选择所在地区" :border="border" :select-open="pickerShow" @click="pickerShow = true"></tn-input>
					</tn-form-item>
					<tn-form-item label="商品类型" prop="goodsType" :label-position="labelPosition" :label-align="labelAlign">
						<tn-input v-model="model.goodsType" type="select" placeholder="请选择商品类型" :border="border" :select-open="selectShow" @click="selectShow = true"></tn-input>
					</tn-form-item>
					<tn-form-item label="验证码" prop="code" :label-position="labelPosition" :label-align="labelAlign">
						<tn-input v-model="model.code" type="text" placeholder="请输入验证码" :border="border"></tn-input>
						<tn-button slot="right" size="sm" background-color="tn-bg-green" font-color="tn-color-white" @click="getCode">{{ codeTips }}</tn-button>
					</tn-form-item>
					<tn-form-item label="记住密码" prop="remember" :label-position="labelPosition" :label-align="labelAlign">
						<tn-switch slot="right" v-model="model.remember"></tn-switch>
					</tn-form-item>
					<tn-form-item label="上传图片" prop="photo" :label-position="labelPosition" :label-align="labelAlign">
						<tn-image-upload :file-list="model.photo" @on-list-change="imageUploadChange"></tn-image-upload>
					</tn-form-item>
				</tn-form>
	...
	data() {
		return {
			...
			rules: {
            	...
            	validator: (rule, value, callback) => this.$t.test.chinese(value),
            	message: '姓名必须为中文',
